...
def main():
    parser = argparse.ArgumentParser(
        usage="%(prog)s [-f FILE] [-p PASS] [--port PORT] [-c HEADER] [-s SECRET] [--cs-install-dir DIR] [--http]"
    )
    ...
    parser.add_argument("--cs-install-dir",      dest="cs_install_dir",  default="/opt",          help="Remote CS install directory")
    ...
    if args.http:
        ...
        if args.cs_install_dir != "/opt": conflicts.append("--cs-install-dir")
        if conflicts:
    ...
    if not args.http:
        ...
        print(f"CS Install Dir:       {args.cs_install_dir}")
    print()
    ...
    cmd = [
        ...
        "-e", f"cs_install_dir={args.cs_install_dir}",
    ]
